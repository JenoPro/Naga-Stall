    ...
    if (Platform.OS === "web") {
      try {
        ...
        try {
          ...
          console.log(
            "ImageManipulator failed on web, using original:",
            manipError
          );
          return imageUri;
        }
  ...
  if (Platform.OS === "web") {
    ...
    try {
      ...
      try {
        ...
        console.log(
          "Supabase client method failed, trying FileSystem upload...",
          supabaseError
        );
        ...
        if (uploadResponse.status !== 200) {
          const errorBody = uploadResponse.body
      ...
      if (formData.image) {
        try {
          ...
          if (imageError.message.includes("Network request failed")) {
            ...
            imageError.message.includes("413") ||
            imageError.message.includes("too large")
          ) {
            errorMessage +=
              "The image file is too large. Please try with a smaller image (under 5MB).";
          } else if (
            imageError.message.includes("401") ||
            imageError.message.includes("403")
          ) {
            errorMessage +=
              "Authentication error. Please restart the app and try again.";
          } else if (
            imageError.message.includes("bucket") ||
            imageError.message.includes("not found")
          ) {
            errorMessage +=
              "Storage configuration error. Please contact support.";
          } else if (imageError.message.includes("file not found")) {
            errorMessage +=
              "Selected image file not found. Please select the image again.";
          } else if (imageError.message.includes("expo-file-system")) {
            errorMessage +=
              "Platform compatibility issue. Please try again or contact support.";
          } else {
